...
const LOG_PREFIX = 'people-updated-event-handler|';
export const handler: SQSHandler = async (event: SQSEvent, context: Context) => {
    ...
    try {
        ...
        logger.debug({ eventType: `${LOG_PREFIX}people`, data: { people } });
        const eventType = FUBEventType.PEOPLE_UPDATED;
        const createdAt = new Date();
        const mapPersonEventId: Record<string, number[]> = records.reduce((acc, { eventId, resourceIds }) => {
            acc[eventId] = resourceIds;
            return acc;
        }, {});
        const allChanges = await findWhatChanges({
            clientId,
            incomingResources: people,
            resourceType: FUBResourceType.PEOPLE,
        });
        const mappedPeople = people.map((person) => {
            ...
            const changes = allChanges.find((change) => change.resourceId === resourceId);
            return {
                clientId,
                fubEventId: eventId,
                personId: resourceId,
